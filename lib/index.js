...
const MOVE_PARAMS = () => ({
  a: { type: 'number', minimum: 0, exclusiveMinimum: true, description: '加速度（米/秒²），默认 1（>0）' },
  v: { type: 'number', minimum: 0, exclusiveMinimum: true, description: '速度（米/秒），默认 1（>0）' },
  t: { type: 'number', minimum: 0, description: '移动时长（秒），默认 0' },
  r: { type: 'number', minimum: 0, description: '交融半径（米），默认 0' },
});
...
export function apply(ctx, config = {}) {
  ...
  const tools = [
    ...
    },
    {
      ...
      parameters: {
        ip: IP(),
        index: { type: 'integer', required: true, minimum: 0, maximum: 23, description: '寄存器下标，范围 [0,23]' },
      },
    },
    {
    ...
    },
    {
      ...
      parameters: {
        ip: IP(),
        index: { type: 'integer', required: true, minimum: 0, maximum: 63, description: '寄存器下标，范围 [0,63]' },
      },
    },
    {
      op: 'list_programs',
    ...
    },
    {
      ...
      parameters: {
        ...
        r: { type: 'number', required: true, minimum: 0, exclusiveMinimum: true, description: '半径（米，>0）' },
    ...
    },
    {
      ...
      parameters: {
        ...
        border: { type: 'number', required: true, minimum: 0, exclusiveMinimum: true, description: '边长（米，>0）' },
        coordinate: { type: 'string', description: '平面："z" 或其它，默认 "z"' },
      },
      timeoutMs: config.commandTimeoutMs,
    },
    {
      ...
      parameters: {
        ip: IP(),
        origin: numberArray({ type: 'number' }, '起点 TCP 位置：[x,y,z,rx,ry,rz]'),
        width: { type: 'number', required: true, minimum: 0, exclusiveMinimum: true, description: '长（米，>0）' },
        height: { type: 'number', required: true, minimum: 0, exclusiveMinimum: true, description: '宽（米，>0）' },
        coordinate: { type: 'string', description: '平面："z" 或其它，默认 "z"' },
      },
      timeoutMs: config.commandTimeoutMs,
    },
    {
      ...
      parameters: {
        ...
        side: { type: 'number', required: true, minimum: 0, exclusiveMinimum: true, description: '边长（米，>0）' },
    ...
    },
    {
      ...
      parameters: {
        ...
        n: { type: 'integer', required: true, minimum: 0, maximum: 15, description: '端口号：std 0-7、config 8-15、tool 0-1' },
      },
    },
    {
      op: 'set_digital_out',
    ...
    },
    {
      ...
      parameters: {
        ip: IP(),
        n: { type: 'integer', required: true, minimum: 0, maximum: 1, description: '端口号（0 或 1）' },
      },
    },
    {
      op: 'set_analog_out',
    ...
    },
    {
      ...
      parameters: {
        ...
        a: { type: 'number', minimum: 0, exclusiveMinimum: true, description: '加速度，默认 1.2（>0）' },
        v: { type: 'number', minimum: 0, exclusiveMinimum: true, description: '速度，默认 0.25（>0）' },
        r: { type: 'number', minimum: 0, description: '交融半径，默认 0' },
      },
      timeoutMs: config.commandTimeoutMs,
    },
    {
    ...
    },
    {
      ...
      parameters: {
        ...
        t: { type: 'number', minimum: 0, description: '控制时长（秒），默认 0.008' },
        lookahead_time: { type: 'number', minimum: 0.03, maximum: 0.2, description: '前瞻时间 0.03-0.2，默认 0.1' },
        gain: { type: 'integer', minimum: 100, maximum: 2000, description: '比例增益 100-2000，默认 100' },
      },
      timeoutMs: config.commandTimeoutMs,
    },
    {
